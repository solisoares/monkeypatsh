...
- `mon patch --default`: patch a command's default invocation directly, without having to open `mon edit` first

### Fixed

- Bash original completion fallback: fix calling completion arguments directly when no original completion was found, e.g. `npm run buil[tab]` was running `npm run buil`
- Type check in `.monrc`: fix incorrect type check when sourcing the monrc file
- Ampersand parsing while rendering: correctly parse `&` in patch code during template rendering
- Backup file validation: `mon restore` now properly rejects invalid or corrupt backup files
...
[v1.1.0]: https://github.com/solisoares/monkeypatsh/compare/v1.0.1...v1.1.0
